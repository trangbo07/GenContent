...
- OUTPUT LANGUAGE: Always write in English, even if source news is in Vietnamese
...
const CUSTOM_SYSTEM_PROMPT = `You are the lead scriptwriter for a world-class FIFA World Cup 2026 daily news show — the broadcast standard of Sky Sports News meets ESPN FC meets BBC Sport.
...
VOICE & RHYTHM
- Write in presenter voice: "Welcome back", "Let's get into it", "Now, here's one that caught everyone's attention..."
- Vary sentence length. Short punchy sentences for impact. Longer flowing ones for narrative depth.
- Use active voice and present tense for current events. Past tense only for completed results.
- Be specific: name players, cite scorelines, reference exact moments.
- Build drama. Every story has a hook, a twist, and a payoff.
- End each section with a verbal bridge that pulls viewers into the next one.

TONE
Authoritative but warm. Excited but not hysterical.

HARD RULES
- Zero bullet points or numbered lists anywhere in the script
- Zero passive voice
- Zero robotic filler phrases
- Zero stage directions, brackets, or meta-text
- Never start two consecutive sentences with the same word
- OUTPUT LANGUAGE: Always write in English — even if source news headlines are in Vietnamese, translate and adapt naturally into English broadcast style

OUTPUT FORMAT
Pure script text only. Section headers exactly as instructed. Nothing else.
Total length: STRICTLY 1,200–1,300 words. Count carefully. Do not exceed 1,300 words under any circumstances.

CRITICAL: Complete every required section before stopping. Never truncate mid-sentence or mid-section.`;
...
// ─── Custom script with manually selected news per section ───────────────────

export interface CustomSectionNews {
  headlines: SelectedNewsItem[];   // Section 1 & 2
  matchResults: SelectedNewsItem[]; // Section 3
  matchOfDay: SelectedNewsItem;    // Section 4
...
function buildCustomPrompt(ctx: { date: string; sections: CustomSectionNews }): string {
  const { sections } = ctx;

  function formatNewsItem(n: SelectedNewsItem, idx: number): string {
    const langNote = n.lang === 'vi' ? ' [Source: Vietnamese — translate naturally to English]' : '';
    return [
      `  [${idx + 1}] ${n.source}${langNote}`,
      `  Headline: ${n.title}`,
      `  Detail: ${n.content.slice(0, 400).replace(/\n/g, ' ')}`,
    ].join('\n');
  }

  const headlinesBlock = sections.headlines.length
    ? sections.headlines.map(formatNewsItem).join('\n\n')
    : '  No headlines selected.';

  const matchResultsBlock = sections.matchResults.length
    ? sections.matchResults.map(formatNewsItem).join('\n\n')
    : '  No match results selected.';

  const motdBlock = [
    `  ${sections.matchOfDay.source}${sections.matchOfDay.lang === 'vi' ? ' [Source: Vietnamese — translate naturally to English]' : ''}`,
    `  Headline: ${sections.matchOfDay.title}`,
    `  Detail: ${sections.matchOfDay.content.slice(0, 600).replace(/\n/g, ' ')}`,
  ].join('\n');

  const section5Instruction = sections.includeSection5
    ? `## SECTION 5: TEAMS & PLAYERS SPOTLIGHT
~170 words. Pick 1–2 player or team story from the headlines above. Specific names, specific moments. One focused paragraph.`
    : `[SECTION 5 IS DISABLED — skip it entirely, do not write it, do not reference it]`;

  const sectionCount = sections.includeSection5 ? 8 : 7;
  const wordTarget = sections.includeSection5
    ? 'With Section 5: aim for 1,250–1,300 words total'
    : 'Without Section 5: aim for 1,200–1,260 words total';

  return `=== WORLD CUP 2026 CUSTOM SCRIPT BRIEF ===
Date : ${ctx.date}
${wordTarget}. STRICTLY between 1,200 and 1,300 words. Count carefully.

━━━ NEWS FOR SECTIONS 1 & 2 (Top Headlines) ━━━
${headlinesBlock}

━━━ NEWS FOR SECTION 3 (Match Results) ━━━
${matchResultsBlock}

━━━ NEWS FOR SECTION 4 (Match of the Day) ━━━
${motdBlock}

NOTE ON SOURCES: If any headline is marked [Source: Vietnamese], you must translate and adapt it naturally into fluent English broadcast language. Never leave Vietnamese text in the script.

═══════════════════════════════════════
WRITE THE SCRIPT BELOW — ${sectionCount} SECTIONS
Use these exact headers. Strict word targets. TOTAL: 1,200–1,300 words.
═══════════════════════════════════════

## SECTION 1: OPENING
~55 words. Open with: "Welcome to World Cup Diary — the hottest stories of ${ctx.date}, here's what's breaking:"
List 2–3 headline teasers as single standalone lines (title only, no detail).
Close with one bridge sentence.

## SECTION 2: TOP WORLD CUP HEADLINES
~250 words. Deliver the full stories from the headlines selected for this section. One tight paragraph per story — hook, key facts, significance.

## SECTION 3: MATCH RESULTS
~110 words. Cover the match results provided. One vivid sentence per result. Fast and energetic.

## SECTION 4: MATCH OF THE DAY
~290 words. Deep dive on the Match of the Day story. Scene-setting, key moments, turning point, standout player, tournament implications. Make the listener feel like they were there.

${section5Instruction}

## SECTION 6: AROUND THE TOURNAMENT
~120 words. One human-interest or lighter story drawn from the headlines. The show's emotional breath before the finale.

## SECTION 7: WORLD CUP STORY OF THE DAY
~210 words. One great moment from World Cup history. Connect it naturally to something happening in 2026. Signature segment — make it memorable.

## SECTION 8: CLOSING
~80 words. Warm, genuine sign-off. Tease the next edition. End on one great line.
The very last sentence must be a standalone "Thank you" — a broadcaster's farewell.

═══════════════════════════════════════
NOW WRITE. Headers exactly as above. Pure script. 1,200–1,300 words total.
═══════════════════════════════════════`;
}

export async function generateCustomScript(
  editionType: EditionType,
  sections: CustomSectionNews,
): Promise<string> {
  const jobId = await createJob(editionType);
  ...
  try {
    ...
    const prompt = buildCustomPrompt({ date, sections });
    const result = await generateScript(prompt, CUSTOM_SYSTEM_PROMPT);
    ...
    const { data: script, error: insertError } = await supabase
      ...
      .insert({
        ...
        metadata: {
          ...
          includeSection5: sections.includeSection5,
          headlineCount: sections.headlines.length,
          matchResultCount: sections.matchResults.length,
        },
        created_at: now,
        updated_at: now,
      })
